...
class AMPOnPolicyRunner:
    def __init__(self, env: VecEnv, train_cfg, log_dir=None, device="cpu"):
        ...
        min_std = torch.tensor(
            self.cfg["amp_min_normalized_std"], device=self.device
        ) * (
            torch.abs(
                self.env.dof_pos_limits[0, :, 1] - self.env.dof_pos_limits[0, :, 0]
            )
        )
    ...
    def learn(self, num_learning_iterations, init_at_random_ep_len=False):
        ...
        cur_amp_sum = torch.zeros(
            self.env.num_envs, dtype=torch.float, device=self.device
        )
        cur_discri_sum = torch.zeros(
            self.env.num_envs, dtype=torch.float, device=self.device
        )
    ...
    def log(self, locs, width=80, pad=35):
        ...
        if len(locs["rewbuffer"]) > 0:
            ...
            self.writer.add_scalar(
                "Train/mean_amp_reward", statistics.mean(locs["ampbuffer"]), locs["it"]
            )
            self.writer.add_scalar(
                "Train/mean_discri_logits",
                statistics.mean(locs["discribuffer"]),
                locs["it"],
            )
            self.writer.add_scalar(
                "Train/mean_amp_reward/time",
                statistics.mean(locs["ampbuffer"]),
                self.tot_time,
            )
            self.writer.add_scalar(
                "Train/mean_discri_logits/time",
                statistics.mean(locs["discribuffer"]),
                self.tot_time,
            )
