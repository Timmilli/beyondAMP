...
class Sim2Sim(ABC):
    ...
    def _init_joint_names(self):
        self.mujoco_joint_names = [
            self.mj_model.jnt(i).name for i in range(self.mj_model.njnt)
        ]
        self.qpos_strat_ids = [
            self.mj_model.jnt_qposadr[i] for i in range(self.mj_model.njnt)
        ]
        self.qvel_strat_ids = [
            self.mj_model.jnt_dofadr[i] for i in range(self.mj_model.njnt)
        ]
    ...
    def _obs_gravity_orientation(self) -> np.ndarray:
        return get_gravity_orientation(
            self.mj_data.qpos[self.base_link_id + 3 : self.base_link_id + 7]
        )
    ...
    def _init_observation_history(self):
        ...
        if self._cfg.observation_cfg.using_base_obs_history:
            ...
            for term, obs_value in initial_obs.items():
                self.base_obs_history[term] = [
                    obs_value.copy()
                    for _ in range(self._cfg.observation_cfg.base_obs_his_length)
                ]
    ...
    def _get_current_base_observations(self) -> dict[str, np.ndarray]:
        ...
        for term in self._cfg.observation_cfg.base_observations_terms:
            if hasattr(self, f"_obs_{term}"):
                base_observations[term] = (
                    getattr(self, f"_obs_{term}")()
                    * self._cfg.observation_cfg.scale[term]
                )
            else:
    ...
    def _update_observation_history(self):
        ...
        if self._cfg.observation_cfg.using_base_obs_history:
            ...
            for term, obs_value in current_obs.items():
                ...
                self.base_obs_history[term] = self.base_obs_history[term][1:] + [
                    obs_value.copy()
                ]
            ...
            for term in self._cfg.observation_cfg.base_observations_terms:
                if term in self.base_obs_history:
                    if self._cfg.observation_cfg.base_obs_flatten:
                        ...
                        base_observations[term] = np.concatenate(
                            self.base_obs_history[term], axis=-1
                        )
                    else:
                        # Keep as separate timesteps: shape (history_length, obs_dim)
                        base_observations[term] = np.stack(
                            self.base_obs_history[term], axis=0
                        )
                else:
