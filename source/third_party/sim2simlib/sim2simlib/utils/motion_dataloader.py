...
class MotionDataloader:
    ...
    def __init__(
        self, dataset: MotionDataset, body_indexes: Sequence[int], device: str = "cuda"
    ):
        ...
        self.motion_offsets: (
            torch.Tensor
        )  # [num_motions], starting index of each motion
        ...
        print(
            f"[Motion_Dataloader] Loading and concatenating {self.num_motions} motions..."
        )
        ...
        print(
            f"[Motion_Dataloader] Initialization complete. Total frames: {self.time_step_total}"
        )
    ...
    def _preload_and_concatenate(self):
        ...
        for i in range(self.num_motions):
            ...
            for key in data_lists.keys():
                data_lists[key].append(
                    torch.tensor(
                        motion_data[key], dtype=torch.float32, device=self.device
                    )
                )
        ...
        self.motion_buffer._body_lin_vel_w = torch.cat(
            data_lists["body_lin_vel_w"], dim=0
        )
        self.motion_buffer._body_ang_vel_w = torch.cat(
            data_lists["body_ang_vel_w"], dim=0
        )
        ...
        self.motion_offsets = torch.cat(
            ...
            dim=0,
        )  # [num_motions], offsets[i] = starting index of motion i

        # Store FPS for each motion
        self.motion_fps = torch.tensor(
            fps_list, dtype=torch.float32, device=self.device
        )
    ...
    def sample(
        self, n: int, weights: torch.Tensor | list | None = None
    ) -> torch.Tensor:
