...
class Sim2SimBeyondMimic(Sim2SimBaseModel):
    ...
    def _init_mimic_dataset(self):
        ...
        self.motion_body_names: list[str] = (
            self._cfg.mimic_dataset_cfg.motion_body_names
        )
        self.dataset_body_names: list[str] = (
            self._cfg.mimic_dataset_cfg.dataset_body_names
        )
        self.policy_body_names: list[str] = self._cfg.policy_body_names

        self.lab_body_indexes = [
            self.policy_body_names.index(name) for name in self.motion_body_names
        ]
        self.motion_anchor_body_index: int = self.dataset_body_names.index(
    ...
    def _robot_anchor_quat_w(self) -> np.ndarray:
        ...
        """
        return self.mj_data.xquat[
            self.mj_anchor_body_id
        ].copy()  # shape: (4,) in (w,x,y,z)
    ...
    def _obs_motion_command(self) -> np.ndarray:
        ...
        joint_pos_target = self.dataloader.motion_buffer.joint_pos[
            self.global_time_steps
        ]
        joint_vel_target = self.dataloader.motion_buffer.joint_vel[
            self.global_time_steps
        ]
    ...
    def _obs_motion_anchor_pos_b(self) -> np.ndarray:
        ...
        anchor_pos_w = self.dataloader.motion_buffer.body_pos_w[
            self.global_time_steps, self.motion_anchor_body_index
        ]  # + self._env.scene.env_origins
        anchor_quat_w = self.dataloader.motion_buffer.body_quat_w[
            self.global_time_steps, self.motion_anchor_body_index
        ]

        # get from mujoco
        robot_anchor_pos_w = torch.tensor(self._robot_anchor_pos_w).to(
            anchor_pos_w.device
        )
        robot_anchor_quat_w = torch.tensor(self._robot_anchor_quat_w).to(
            anchor_quat_w.device
        )
    ...
    def reset(self, motion_id):
        ...
        # lab order
        init_joint_pos = (
            self.dataloader.motion_buffer.joint_pos[self.global_time_steps]
            .cpu()
            .numpy()
            .flatten()
        )
        init_joint_vel = (
            self.dataloader.motion_buffer.joint_vel[self.global_time_steps]
            .cpu()
            .numpy()
            .flatten()
        )
        init_root_pos = (
            self.dataloader.motion_buffer.body_pos_w[self.global_time_steps, 0]
            .cpu()
            .numpy()
            .flatten()
        )
        init_quat = (
            self.dataloader.motion_buffer.body_quat_w[self.global_time_steps, 0]
            .cpu()
            .numpy()
            .flatten()
        )
        ...
        with mujoco.viewer.launch_passive(self.mj_model, self.mj_data) as viewer:
            while viewer.is_running():
                ...
                time_until_next_step = (
                    self.mj_model.opt.timestep * self.slowdown_factor
                    - (time.time() - step_start)
                )
                if time_until_next_step > 0:
                    time.sleep(time_until_next_step)
        ...
        if headless:
            ...
            with mujoco.viewer.launch_passive(self.mj_model, self.mj_data) as viewer:
                ...
                while viewer.is_running():
                    ...
                    init_root_pos = (
                        self.dataloader.motion_buffer.body_pos_w[
                    ...
                    init_quat = (
                        self.dataloader.motion_buffer.body_quat_w[
                            self.global_time_steps, 0
                        ]
                        .cpu()
                        .numpy()
                    ...
                    if (
                        self.time_steps
                        < self.dataloader.motion_lengths[self.motion_id] - 1
                    ):
                        self.global_time_steps += 1
                        self.time_steps += 1
                    else:
                        cur_motion_id = (
                            cur_motion_id + 1
                        ) % self.dataloader.num_motions
                        self.reset(motion_id=cur_motion_id)
                        rich.print(
                            f"[Sim2SimMimic] Switching to motion id: {cur_motion_id}"
                        )

                    time_until_next_step = (
                        self.mj_model.opt.timestep * self._cfg.control_decimation
                        - (time.time() - step_start)
                    )
                    if time_until_next_step > 0:
                        time.sleep(time_until_next_step)
