...
class ActionFluctuationRatioCommand(NullCommand):
    ...
    def __init__(self, cfg: ActionFluctuationRatioCommandCfg, env: ManagerBasedRLEnv):
        ...
        self.metrics["action_fluctuation_ratio"] = torch.zeros(
    ...
    def _update_metrics(self):
        ...
        if torch.any(valid_envs):
            ...
            if self.cfg.using_term:
                cur_action = self._env.action_manager._terms[
                    self.cfg.term_name
                ]._raw_actions
                prev_action = self._env.action_manager._terms[
                    self.cfg.term_name
                ]._prev_raw_actions
            else:
                cur_action = self._env.action_manager._action  # (num_envs, action_dim)
                prev_action = (
                    self._env.action_manager._prev_action
                )  # (num_envs, action_dim)
