...
parser.add_argument(
    "--target", type=str, default=None, help="If use, direct point to the target ckpt"
)
...
def main():
    ...
    if task_name is None:
        assert os.path.exists(os.path.join(run_path, "params", "args.pkl")), (
            "No task specified."
        )
    ...
    export_policy_as_onnx(
        runner.alg.actor_critic, export_model_dir, filename="policy.onnx"
    )
    export_policy_as_jit(
        runner.alg.actor_critic, None, export_model_dir, filename="policy.pt"
    )
